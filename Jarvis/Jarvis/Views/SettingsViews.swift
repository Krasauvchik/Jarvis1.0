...
private enum BackendStatus {
    ...
    var color: Color {
        switch self {
        case .online: return JarvisTheme.accentGreen
...
struct SettingsContent: View {
    ...
    // Backend settings
    @State private var backendURL: String = UserDefaults.standard.string(forKey: Config.Storage.backendURLKey) ?? ""
    @State private var apiKey: String = SecretStore.get(.backendAPIKey) ?? ""
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 20) {
                ...
                settingsSection(title: L10n.sidebarModules, icon: "slider.horizontal.3") {
                    SidebarModulesSettingsView(theme: theme)
                }
                
                settingsSection(title: L10n.settingsAISection, icon: "brain.head.profile") {
                    AIProviderSettingsView(theme: theme, aiManager: aiManager)
                }
                ...
                settingsSection(title: "Backend", icon: "server.rack") {
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        HStack {
                            ...
                            Button(action: saveBackendURL) {
                                Image(systemName: "checkmark.circle.fill")
                                    .foregroundColor(JarvisTheme.accent)
                            }
                            .buttonStyle(.plain)
                        }
                        Text("Leave empty for default. Current: \(Config.backendBase)")
                            .font(.system(size: 11))
                            .foregroundColor(theme.textTertiary)
                    }
                    .padding(.vertical, 4)
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        Text("Must match JARVIS_API_KEY on the server")
                            .font(.system(size: 11))
                            .foregroundColor(theme.textTertiary)
                    }
                    .padding(.vertical, 4)
                    ...
                    HStack {
                        Button(action: checkBackendHealth) {
                ...
                settingsSection(title: L10n.settingsData, icon: "externaldrive.fill") {
                    ...
                    Button(action: {
                        ...
                        HStack {
                            Label(L10n.settingsExportCSV, systemImage: "tablecells")
                                .foregroundColor(theme.textPrimary)
                            Spacer()
                        }
                        .padding(.vertical, 8)
    ...
    private func saveBackendURL() {
        ...
        UserDefaults.standard.set(trimmed, forKey: Config.Storage.backendURLKey)
    }
    
    private func saveAPIKey() {
        SecretStore.set(apiKey, for: .backendAPIKey)
    }
    
    private func checkBackendHealth() {
        backendStatus = .checking
        guard let url = URL(string: "\(Config.backendBase)/health") else {
            backendStatus = .offline
            return
        }
        var req = Config.authorizedRequest(url: url)
        req.timeoutInterval = 8
        Config.urlSession.dataTask(with: req) { _, response, error in
